  ...
  await Promise.all(files.map(async (file) => {
    ...
    try {
      const minified = await minifyHtml(content, {
        ...
        minifyCSS: true,
        minifyJS: true,
        ignoreCustomFragments: [/<\?[\s\S]*?\?>/], // don't break PHP
      });
...
async function processCssFiles() {
  ...
  const purge = purgecss({
    ...
    safelist: {
      standard: [
        ...
        /^navbar(-.*)?$/, /^d-(sm|md|lg|xl|xxl)?-(flex|block|none)$/,
        /^show$/, /^fade$/, /^active$/, /^carousel(-.*)?$/
      ]
    }
  });
  ...
  await Promise.all(cssFiles.map(async (file) => {
    ...
    const out = await postcss([purge, cssnano({ preset: 'default' })]).process(css, { from: file, to: file });
...
async function optimizeImages() {
  ...
  await Promise.all(imgFiles.map(async (srcPath) => {
    ...
    await Promise.all(IMG_WIDTHS.map(async (w) => {
      await sharp(srcPath).resize({ width: w, withoutEnlargement: true })
